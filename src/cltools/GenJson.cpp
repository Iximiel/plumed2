...
int GenJson::main(FILE* in, FILE*out,Communicator& pc) {
  ...
  for(unsigned i=0; i<action_names.size(); ++i) {
    ...
    for(unsigned j=0; j<keys.size(); ++j) {
      ...
      if( mydescrip.find("\\")!=std::string::npos ) error("found invalid backslash character documentation for keyword " + keywords[j] + " in action " + action_names[i] );
      std::string argtype = keys.getArgumentType( keywords[j] );
      if( argtype.length()>0 ) std::cout<<"       \""<<keywords[j]<<"\" : { \"type\": \""<<keys.getStyle(keywords[j])<<"\", \"description\": \""<<mydescrip<<"\", \"multiple\": "<<keys.numbered( keywords[j] )<<", \"argtype\": \""<<argtype<<"\"}";
      else if( defa.length()>0 ) std::cout<<"       \""<<keywords[j]<<"\" : { \"type\": \""<<keys.getStyle(keywords[j])<<"\", \"description\": \""<<mydescrip<<"\", \"multiple\": "<<keys.numbered( keywords[j] )<<", \"default\": \""<<defa<<"\"}";
      else std::cout<<"       \""<<keywords[j]<<"\" : { \"type\": \""<<keys.getStyle(keywords[j])<<"\", \"description\": \""<<mydescrip<<"\", \"multiple\": "<<keys.numbered( keywords[j] )<<"}";
