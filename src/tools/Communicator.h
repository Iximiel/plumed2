...
/// Init from pointer to VectorGeneric
    template <typename T, unsigned n> explicit Data(VectorGeneric<T,n> *p,int s): pointer(p), size(n*s), nbytes(sizeof(double)), type(getMPIType<double>()) {}
/// Init from reference to VectorGeneric
    template <typename T, unsigned n> explicit Data(VectorGeneric<T,n> &p): pointer(&p), size(n), nbytes(sizeof(double)), type(getMPIType<double>()) {}
...
/// See Communicator::Data documentation
  struct ConstData {
    ...
    template <typename T,unsigned n> explicit ConstData(const VectorGeneric<T,n> *p,int s): pointer(p), size(n*s), nbytes(sizeof(double)), type(getMPIType<T>()) {}
    template <typename T,unsigned n> explicit ConstData(const VectorGeneric<T,n> &p): pointer(&p), size(n), nbytes(sizeof(double)), type(getMPIType<T>()) {}
