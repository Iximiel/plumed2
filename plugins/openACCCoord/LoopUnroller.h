...
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
#ifndef __PLUMED_tools_LoopUnrollerFloat_h
#define __PLUMED_tools_LoopUnrollerFloat_h
...
operations with unrolled loops. The methods work on T*
...
All the methods for class template<n,  T> act on n elements.
Implementation is made using template metaprogramming, that is:
- LoopUnroller<1,  T>::xxx acts on the element [0] of the array.
- template<n,  T>::xxx calls LoopUnroller<n-1,  T>::xxx then acts on element [n-1] of the array.
...
template<unsigned n, typename T>
void LoopUnroller<n,T>::_zero(T*d) {
  if constexpr (n>1) {
    LoopUnroller<n-1,T>::_zero(d);
  }
  d[n-1]=T(0);
}

template<unsigned n, typename T>
void LoopUnroller<n,  T>::_add(T*d,const T*a) {
  if constexpr (n>1) {
    LoopUnroller<n-1,  T>::_add(d,a);
  }
  d[n-1]+=a[n-1];
}

template<unsigned n, typename T>
void LoopUnroller<n,  T>::_sub(T*d,const T*a) {
  if constexpr (n>1) {
    LoopUnroller<n-1,  T>::_sub(d,a);
  }
  d[n-1]-=a[n-1];
}

template<unsigned n, typename T>
void LoopUnroller<n,  T>::_mul(T*d,const T s) {
  if constexpr (n>1) {
    LoopUnroller<n-1,  T>::_mul(d,s);
  }
  d[n-1]*=s;
}

template<unsigned n, typename T>
void LoopUnroller<n,  T>::_neg(T*d,const T*a ) {
  if constexpr (n>1) {
    LoopUnroller<n-1,  T>::_neg(d,a);
  }
  d[n-1]=-a[n-1];
}

template<unsigned n, typename T>
T LoopUnroller<n,  T>::_sum2(const T*d) {
  if constexpr (n>1) {
    return LoopUnroller<n-1,  T>::_sum2(d)+d[n-1]*d[n-1];
  } else {
    return d[0]*d[0];
  }
}
template<unsigned n, typename T>
T LoopUnroller<n,  T>::_dot(const T*d,const T*v) {
  if constexpr (n>1) {
    return LoopUnroller<n-1,  T>::_dot(d,v)+d[n-1]*v[n-1];
  } else {
    return d[0]*v[0];
  }
}
