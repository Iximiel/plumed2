  ...
  if(getStep()%NL.stride==0&& mode != calculationMode::pair) {
    ...
    case calculationMode::self: {
      ...
      if (mpiActive) {
        ...
        plumed_assert(maxExpected <= maxNumberOfAtomsPerCell)
            << "The number of atoms exceed the current compute capability, try by reducing the cell dimensions";
    ...
    case calculationMode::dual: {
      ...
      if (mpiActive) {
        ...
        plumed_assert(maxExpected_dev <= maxNumberOfAtomsPerCell)
            << "The number of atoms exceed the current compute capability, try by reducing the cell dimensions";
        //the largest cell combination in listA
        const auto maxExpected_coord=cs.listB.getMaximimumCombination(27);
        plumed_assert(maxExpected_coord <= maxNumberOfAtomsPerCell)
            << "The number of atoms exceed the current compute capability, try by reducing the cell dimensions";
        ...
        cellConfiguration_coord.reset(cells.getNumberOfCells(),
                                      ...
                                      nat);
        cellConfiguration_dev.reset(cells.getNumberOfCells(),
                                    biggestCell_dev,
                                    //the higher index in listB
                                    nat,
                                    maxExpected_dev,
...
//TODO: measure if using the number of atoms in the inner cell is faster
  const unsigned threads = std::min(maxNumThreads,cellConfiguration_coord.biggestCell);
  ...
  plumed_assert(memSize <= maxDynamicSharedMemory) << "the shared memory asked exceed the limit for this GPU:"
      << memSize << " > " << maxDynamicSharedMemory;
...
//  cellSetup cellConfiguration_coord(cells.getNumberOfCells(),biggestCell_coord,atomsInA,maxExpected_coord,nat);
  ...
  /*  plumed_assert(biggestCell_dev <= maxNumThreads) << "the number of atoms in a single cell exceds the maximum number of threads avaiable";
    ...
    plumed_assert(memSize_coord <= maxDynamicSharedMemory) << "the shared memory asked exceed the limit for this GPU:"
        << memSize_coord << " > " << maxDynamicSharedMemory;
    plumed_assert(memSize_dev <= maxDynamicSharedMemory) << "the shared memory asked exceed the limit for this GPU:"
        << memSize_dev << " > " << maxDynamicSharedMemory;*/
  const unsigned ngroups = cells.getNumberOfCells();
  const unsigned threads_coord = std::min(maxNumThreads,cellConfiguration_coord.biggestCell);
  const unsigned threads_dev = std::min(maxNumThreads,cellConfiguration_dev.biggestCell);
...
#define defParse(pn) std::string pn ## s=pn ## default;\
  ...
  if (mpiActive) {
    ...
    if(mode == calculationMode::dual) {
      ...
      maxNumThreads_dualDev = min (attr.maxThreadsPerBlock, maxNumThreads_dualDev);
      maxDynamicSharedMemory_dualDev=attr.maxDynamicSharedSizeBytes;
      maxNumberOfAtomsPerCell_dev = maxDynamicSharedMemory_dualDev / (3*sizeof(calculateFloat)+sizeof(unsigned));
    }
  }
  maxNumberOfAtomsPerCell = maxDynamicSharedMemory / (3*sizeof(calculateFloat)+sizeof(unsigned));
  ...
  if (mode == calculationMode::dual) {
    log<< "\t max threads per coordination(derivatives loop)" << maxNumThreads_dualDev << "\n";
  }

  if (mode == calculationMode::dual || mode == calculationMode::self) {
    log <<"\t max number of atoms in neighbour cells: " << maxNumberOfAtomsPerCell << "\n";
    if (mode == calculationMode::dual) {
      log <<"\t max number of atoms in neighbour cells(derivatives loop): " << maxNumberOfAtomsPerCell_dev << "\n";
    }
  }
