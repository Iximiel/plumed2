...
int main(){
  VectorGeneric<T, 3> v1;
  v1[0]=3.0;
// use equivalently () and [] syntax:
  v1(1)=5.0;
// initialize with components
  VectorGeneric<T, 3> v2=VectorGeneric<T, 3>(1.0,2.0,3.0);
  VectorGeneric<T, 3> v3=crossProduct(v1,v2);
...
/// return s*v
  template<typename U, unsigned m>
  friend VectorGeneric<U, m> operator*(U,VectorGeneric<U, m>);
/// return v*s
  template<typename U, unsigned m>
  friend VectorGeneric<U, m> operator*(const VectorGeneric<U, m>&,U);
/// return v/s
  template<typename U, unsigned m>
  friend VectorGeneric<U, m> operator/(const VectorGeneric<U, m>&,U);
/// return v2-v1
...
/// return v1 .scalar. v2
  template<typename U, unsigned m>
  friend T dotProduct(const VectorGeneric<U, m>&,const VectorGeneric<U, m>&);
...
/// Only available for size 3
  friend VectorGeneric<T, 3> crossProduct(const VectorGeneric<T, 3>&,const VectorGeneric<T, 3>&);
...
/// friend version of modulo2 (to simplify some syntax)
  template<unsigned m>
  friend T modulo2(const VectorGeneric<T, m>&);
/// friend version of modulo (to simplify some syntax)
  template<unsigned m>
  friend T modulo(const VectorGeneric<T, m>&);
/// << operator.
...
template<typename T, unsigned n>
VectorGeneric<T, n> operator*(T s,VectorGeneric<T, n>v) {
  return v*=s;
}

template<typename T, unsigned n>
VectorGeneric<T, n> operator*(const VectorGeneric<T, n>&v,T s) {
  return s*v;
}
...
using Vector3d = VectorGeneric<T, 3>;
/// \ingroup TOOLBOX
